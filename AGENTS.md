...
* 配色：紫 / 靛蓝 / 蓝紫渐变（`#6366F1`、`#8B5CF6`）；纯平背景（必须噪点纹理或渐变）；Tailwind 默认色板。
* 布局：Hero + 三卡片；完美居中；等宽多栏（必须不对称）。
* 文案：高深名词和空话；Lorem Ipsum；被动语态和长句。风格：口语化、有数字和场景、可幽默自嘲、每句 ≤15 字。
* 组件：Shadcn / Material UI 默认组件（必须深度定制）；Emoji 当功能图标；线性动画（`ease-in-out`）。
* 模板味：Inter + 白底 + 紫渐变 + 网格卡片 + 圆角堆叠——这套"AI 默认脸"禁用。
...
**设计审计**（页面 / 组件完成后，截图逐条自查）：

* 去掉 logo 还能认出是"一盏茶"吗？认不出 = 没性格。
* 会不会被当成模板 / AI 生成？会就改掉最像模板的那个元素。
* 数字、场景、文案是否具体？有没有"提升用户体验"这类空话？
* 动效是否有意图（引导注意 / 反馈状态）？还是纯装饰？
