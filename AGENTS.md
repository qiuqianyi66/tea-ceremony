...
> 验收标准：**一眼认不出是模板，也认不出是 AI 生成的**。

* 设计任务必读根目录 `FRONTEND_DESIGN_SPEC.md`（四家融合的通用规范：Design Read → 三拨盘 → 体系选择 → 禁令 → 质量底线 → 设计审计）+ `.agents/skills/frontend-design/SKILL.md`（Anthropic 官方方法论）。
* 开工先输出一行 Design Read（页面类型 / 受众 / vibe / 倾向体系），再设三拨盘（VARIANCE / MOTION / DENSITY，基线 8/6/4），再动手。

**禁令速查**（完整清单见 FRONTEND_DESIGN_SPEC.md §3）：

* 配色：AI 紫渐变、暖米白 + 陶土、纯黑灰（要 tint）、Tailwind 默认色板；**一页一个强调色全页锁定**。
* 字体：Inter / 系统栈禁默认；**衬线体禁默认**（最大 AI 特征）；Fraunces / Instrument_Serif 直接禁。
* 布局：Hero + 三卡片；完美居中；等宽多栏；**眉题 eyebrow 绝对禁**；01/02/03 编号除非真序列。
* 组件：卡片套卡片；玻璃拟态装饰；渐变文字；彩色侧边条；Emoji 当图标；几何蒙版。
* 动效：每节同款入场禁；bounce / elastic 禁；悬停动图禁；**一个编排时刻**。
* 文案：空话 / Lorem Ipsum / 被动语态 / 错误道歉禁；按钮写动作不写"提交"。风格：口语化、有数字和场景、每句 ≤15 字。
...
* **更新记录**：2026-09-09 融合多源规范重构（行为脚手架 + 边界三层 + 学习记录 + 设计风格规范并入）；同日前端设计升级为「反主流 × 创新」规范（视觉方向先行 + 2026 创新工具箱 + 设计审计）。
