...
/**
 * TeaBrewScene3D — 冲泡页 3D 真实感茶席场景（TresJS 声明式）
 *
 * 视觉风格：夜色暖光·炭火煮茶（暖黄纸灯、炭火陶炉、暗部沉稳、柔光）。
 * 茶具程序化建模（零外部素材）：盖碗 + 茶壶 + 炉 + 蒸汽粒子 + 炉火。
 * 通过 props 与 BrewView 状态机联动：phase 驱动阶段视觉、soupColor 驱动茶汤色、
 * currentTemp 驱动蒸汽与炉火强度。不修改任何业务逻辑。
 */
...
// ==================== 坐标常量（TresJS position/rotation/scale 需 Vector3） ====================
...
const floorRot = new THREE.Vector3(-Math.PI / 2, 0, 0)
const tablePos = new THREE.Vector3(0, 1.1, 0)
const clothPos = new THREE.Vector3(0, 1.19, 0)
const clothRot = new THREE.Vector3(-Math.PI / 2, 0, 0)
...
const spoutRot = new THREE.Vector3(0, 0, -Math.PI / 5)
const handlePos = new THREE.Vector3(-1.35, 0.95, 0)
const handleRot = new THREE.Vector3(0, 0, Math.PI / 2.3)
...
// 火焰片布局（含位置与绕 Z 旋转）
const flameSlots: { pos: THREE.Vector3; rot: THREE.Vector3 }[] = [
  { pos: new THREE.Vector3(-0.32, 0.6, 0.05), rot: new THREE.Vector3(0, 0, -0.12) },
  { pos: new THREE.Vector3(-0.16, 0.6, -0.05), rot: new THREE.Vector3(0, 0, 0.06) },
  { pos: new THREE.Vector3(0, 0.6, 0), rot: new THREE.Vector3(0, 0, 0) },
  { pos: new THREE.Vector3(0.16, 0.6, -0.05), rot: new THREE.Vector3(0, 0, -0.06) },
  { pos: new THREE.Vector3(0.32, 0.6, 0.05), rot: new THREE.Vector3(0, 0, 0.12) },
]
...
<template>
  <div class="pointer-events-none absolute inset-0 z-0 overflow-hidden" aria-hidden="true">
    <TresCanvas :clear-color="'#1c1612'" :antialias="true">
      <!-- 相机 -->
      ...
      <TresMesh :position="floorPos" :rotation="floorRot">
      ...
      <TresMesh :position="clothPos" :rotation="clothRot">
      ...
      <TresGroup ref="kettleGroup" :position="kettlePos" :scale="kettleScale">
        ...
        <TresMesh :position="spoutPos" :rotation="spoutRot">
          <TresCylinderGeometry :args="[0.16, 0.3, 1.1, 16]" />
          <TresMeshStandardMaterial :color="'#8a6b48'" :roughness="0.55" :metalness="0.05" />
        </TresMesh>
        <TresMesh :position="handlePos" :rotation="handleRot">
