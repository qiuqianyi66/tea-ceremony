...
// 十二境流程步骤（icon 为全局注册的 lucide 组件名，禁止 emoji 当图标）
const teaSteps = [
  { icon: 'Sparkles', name: '净手', desc: '洗净尘埃，心随水静' },
  { icon: 'DoorOpen', name: '入席', desc: '端坐茶席前，收摄心神' },
  { icon: 'Eye', name: '观茶', desc: '观干茶之形，闻茶香之韵' },
  { icon: 'CupSoda', name: '温器', desc: '温热茶器，以迎茶汤' },
  { icon: 'Droplet', name: '取水', desc: '好水为茶之母' },
  { icon: 'Flame', name: '烹水', desc: '听水声三沸：蟹眼、鱼眼、松涛' },
  { icon: 'Leaf', name: '投茶', desc: '投茶入器，量随人定' },
  { icon: 'Waves', name: '注水', desc: '环壁低斟，唤醒茶香' },
  { icon: 'Timer', name: '浸泡', desc: '静待茶汤渐成' },
  { icon: 'Beaker', name: '出汤', desc: '茶汤入杯，香韵初显' },
  { icon: 'Gem', name: '观色', desc: '观汤色之变化，赏茶韵之流转' },
  { icon: 'Coffee', name: '品味', desc: '入口、回味、余韵，三品而知茶' },
]
        ...
        <div class="relative">
          <button @click="showRoomPicker = !showRoomPicker" aria-label="选择茶室" :aria-expanded="showRoomPicker"
            class="text-sm text-[var(--color-wood)] hover:text-[var(--color-wood-light)]">
                        <component :is="iconOf(theme.currentTheme.icon)" :size="16" class="inline-block mr-1 -mt-0.5" /> {{ room.currentRoom.name }}
          </button>
          <div v-if="showRoomPicker" class="absolute top-8 left-0 glass-panel rounded-xl p-2 w-44 shadow-xl z-20"
            ...
            <button v-for="t in TEA_ROOM_THEMES" :key="t.id" @click="room.setRoom(t.id); showRoomPicker = false"
              ...
              <component :is="iconOf(t.icon)" :size="16" class="inline-block mr-2 -mt-0.5" />{{ t.name }}
            </button>
          </div>
        </div>
      </div>
    ...
    <Teleport to="body">
      <div v-if="showGreeting"
        ...
        <div class="glass-panel rounded-2xl p-8 max-w-sm text-center animate-[fadeIn_0.5s_ease-out] pointer-events-auto" @click.stop>
          <IconCupSoda :size="44" class="mx-auto mb-4" />
