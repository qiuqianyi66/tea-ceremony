  ...
  for (const region of ['hangzhou', 'wuyishan', 'yunnan', 'fuding']) {
    ...
    const proj = await page.evaluate(() => {
      try {
        ...
        const down = new g.THREE.Raycaster(
          new g.THREE.Vector3(12, 60, 8),
          new g.THREE.Vector3(0, -1, 0),
        )
        let terrain = null
        g.scene().traverse((o) => {
          if (o.isMesh && o.material && o.material.vertexColors === true) terrain = o
        })
    ...
    if (!proj.ndc) {
      console.log(region, 'NO PROJ', JSON.stringify(proj))
      await page.close()
      continue
    }
    ...
    console.log(
      region,
      `PROJ ndc=(${proj.ndc[0].toFixed(2)},${proj.ndc[1].toFixed(2)}) h=${proj.h.toFixed(1)} Click=(${cx.toFixed(0)},${cy.toFixed(0)})`,
    )
    await page.mouse.click(cx, cy)
    await page.waitForTimeout(1500)

    const panelVisible = await page
      .locator('.pavilion-panel')
      .isVisible()
      .catch(() => false)
