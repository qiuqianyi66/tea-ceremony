...
;(async () => {
  ...
  const ctrl = await page.evaluate(() => {
    ...
    return {
      ...
      hasListeners: c.domElement
        ? c.domElement._listeners
          ? Object.keys(c.domElement._listeners).length
          : 'n/a'
        : null,
    }
  })
  ...
  const camBefore = await page.evaluate(() => {
    ...
    const c = cam && cam.value ? cam.value : cam
  ...
  for (let i = 0; i < 10; i++) {
    await page.mouse.move(720 + i * 12, 450 + i * 6)
  ...
  if (wb) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      waterState = await page.evaluate(() => {
        ...
        let pts = null
        scene.traverse((o) => {
          if (o.type === 'Points' && o.geometry && o.geometry.attributes.position) pts = o
        })
