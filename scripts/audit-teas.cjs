...
for (const m of s.matchAll(re)) {
  entries.push({ id: m[1], name: m[2], type: m[3] })
}
...
console.log(
  'teaMasters.relatedTeas 断链:',
  [...new Set(related.filter((x) => !ids.has(x)))].join(',') || '无',
)
...
console.log(
  'gardenRegions.teaIds 断链:',
  [...new Set(teaIds.filter((x) => !ids.has(x)))].join(',') || '无',
)

// 图片引用 vs 文件存在
const imgMap = {}
for (const im of s.matchAll(/import (\w+) from '(@\/assets\/[^']+)'/g))
  imgMap[im[1]] = im[2].replace('@/', 'src/')
