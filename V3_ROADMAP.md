...
| 设计令牌 | AGENTS.md 已立禁令与晨雾茶山色系 | 全局一致性检查：色令牌、字体角色、浏览器表面定制 | P0 | `node scripts/scan-emoji.cjs` 零命中；逐页截图对比令牌；正文对比度 ≥ 4.5:1 |
...
### 4.2 视觉方向：晨雾茶山体系（色令牌 + 字体角色）

| 令牌 | 色值 | 角色 |
|---|---|---|
| 宣纸白 | #F5F1E6 | 页面底色 |
| 墨青 | #0F1A14 ~ #1A2420 | 主文字/深色块 |
| 茶汤金 | #C9A96E | 强调色（一页一个强调色，全页锁定） |
| 竹青 | #A5C9A0 | 次级强调/状态 |
| 枯褐 | #8D6E63 | 辅助/描边 |
...
| P0-3 设计令牌落地 | 晨雾茶山色令牌/字体角色全局一致性检查 | `node scripts/scan-emoji.cjs` 零命中；逐页截图对比色令牌；正文对比度 ≥ 4.5:1；浏览器表面定制（选区/滚动条/焦点环） |
