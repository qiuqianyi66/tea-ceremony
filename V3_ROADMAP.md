...
|---|---|---|---|---|
| 首页沉浸 | Hero 视频 + 入席，三级降级 | ✅ 已收尾：preload 修复（df4ec69）；离线/拦截降级静态图已有 | 完成 | Playwright 截图确认视频渲染；离线/拦截时静态图降级生效 |
| 视觉升级 | 3D 茶席/茶园已多轮迭代；泡茶动画、产区地图、晨雾茶山已有落点 | ✅ 已过检：P0-1 逐页审计 28+2 截图全过（f24d678/955881a）；verify-gardens ERRORS:[] | 完成 | `node scripts/verify-gardens.cjs` 期望 ERRORS: []；改前/改后截图对比；设计审计清单全过（第 4 章） |
...
| 测试覆盖 | 81 Vitest + 13 E2E + 后端 34 过 1 跳 + verify-* 脚本群 | ✅ 已完成：分享链路 E2E 9 用例（1838801）；四方向审计脚本入库；test_culture 4 用例（3697d9b）；test_records 防 307（5f76ba7）；scoring explain 3 用例（b254bc7）；E2E 全量 13/13 实跑通过（本轮） | 完成 | 改动提交前跑对应 verify 脚本；分享卡 E2E 覆盖编码/解码/非法输入；后端 pytest 全量过 |
...
| 项 | 目标 | 验收标准 |
|---|---|---|
| P1-1 茶文化深度 ✅ | 产区风土叙事入茶详情（544621e）；冲泡参数 49 款全量比对，30 款差异全归因为合理款级差异 | 抽样 10 款茶参数与基准表一致，不一致修复并标来源；无"待核实"悬空 |
...
| P1-7 边界/规范全量审计 ✅ | J 批次三轮：AIAsk/TeaBreak 边界（9c2cdc5/8880559 呼吸 rAF+输入上限+绿茶茶文本修复）、茶柜/历史违规范项（305b1e2 emoji/裸!图标/纯灰/评分口径）、any 清零+手绘 SVG+lucide+表面定制（e0c0686）；10 页视觉审计 0 违规（913a5ff）；备器页守卫行为验证 | type-check/build/81 单测/后端 34 过 1 跳全绿；E2E 13/13；audit-touch/scan-emoji 零命中；全局 any 计数 0 |
