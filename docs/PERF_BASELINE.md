...
| CSS | 0.12 MB | 单文件 |
| 图片（非 3D） | ≈1.5 MB | tearoom-bg 0.14 / hero 0.29 / porcelain 0.62 / zisha 0.54 / 茶卡·茶园图（2D 背景图已压缩，见下） |
| 3D 纹理 | 34.2 MB | 8 张 2K 地形贴图（terrain/*），**不进 precache**，运行时 CacheFirst 按需加载 |
| **离线包（precache）** | ≈4 MB | JS + CSS + 非 3D 图片（3D 纹理排除，见 vite.config globIgnores `**/3d/**`） |
...
| 总传输 | 878 KB | 首页首屏全量（压缩前 1187KB，**-26%**） |
| 图片 | 407 KB | **已优化**（压缩前 716KB，**-43%**）：Hero 图 599→290KB（q=70）、tearoom-bg 684→145KB（q=80，mozjpeg） |
...
- ✅ **图片首屏优化已落地**：716KB → 407KB（压缩 2 张 2D 背景图，3D 贴图未动以保渲染质量）；截图复核首页/茶室视觉无损
- ⚠️ **3D 纹理 34MB 是 dist 部署体积大头**：属设计权衡（2K 贴图质量 vs 体积），已按需加载不影响首屏；若 GitHub Pages 部署空间敏感可降 1K 纹理（代价是 3D 地面细节）
- ℹ️ 离线包 ≈4MB 对 PWA 可接受；3D 页首次访问时纹理走运行时缓存（网络可达时）
